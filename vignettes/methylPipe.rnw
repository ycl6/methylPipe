...
and fetal lung fibroblasts (IMR90). The methylation data can be stored into \Rclass{BSdata} objects and then collected together within a \Rclass{BSdataSet} object with the following commands:
<<met1,message=FALSE,cache=TRUE>>=
...
\begin{center}
<<met3,fig.width=6,fig.height=5,out.width='.85\\textwidth',message=FALSE,cache=TRUE>>=
...
It is an extension of \Rclass{SummarizedExperiment} class of \Rclass{GenomicRanges} package. 
...
plotMeth(gel, datatype=c("mC","mC"), yLim=c(.025, .025), brmeth=H1.IMR90.set,
         mcContext="CG", Datatrackname=c("Abs meth","Abs meth","H1","IMR90"), 
         transcriptDB=txdb, chr="chr20", org=Hsapiens)
@
